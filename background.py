...
#represents the different outcomes of result due to setting parameters for the Pokemon class
  ...
  if choice == '0':
    ...
    pick = Pokemon("Whirlipede", choice, bug+"/"+poi, "Venipede", "Scolipede",whirlipede)
    window.fill(poiC)
    ...
    pick = Pokemon("Duosion", choice, psy, "Solosis", "Reuniclus", duosion)
    window.fill(psyC)
    ...
    pick = Pokemon("Vanillish", choice, ice, "Vanillite", none,vanillish)
    window.fill(iceC)
    ...
    pick = Pokemon("Deerling", choice, nor+"/"+gra, none, "Sawsbuck",deerling)
    window.fill(graC)
    ...
    pick = Pokemon("Karrablast", choice, bug, none, "Escavalier",karrablast)
    window.fill(bugC)
